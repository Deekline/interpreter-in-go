...
func TestReturnStatement(t *testing.T) {
  ...
  `
	...
	if len(program.Statements) != 3 {
		t.Errorf("program.Statements does not contain 3 return statements. got=%d", len(program.Statements))
	}
...
func testLetStatement(t *testing.T, s ast.Statement, name string) bool {
	...
	if letStmt.Name.Value != name {
		t.Errorf("letStmt.Name.Valu not '%s' got =%s", name, letStmt.Name.Value)
		return false
	}
...
func TestParsingPrefixExpressions(t *testing.T) {
	...
	for _, tt := range prefixTests {
		...
		if len(program.Statements) != 1 {
			t.Fatalf("program has not enogh statements %d statements. got=%d\n", 1, len(program.Statements))
		}
		stmt, ok := program.Statements[0].(*ast.ExpressionStatement)
		if !ok {
			t.Fatalf("program.Statements[0] is not ast.ExpressionStatement. got=%T", program.Statements[0])
